...
Architecture ported (inference-only) from:
    https://github.com/minivision-ai/Silent-Face-Anti-Spoofing

Uses a pruned MobileFaceNet backbone with depthwise-separable convolutions.
Outputs a 3-class logit vector (background / real / spoof) that is
softmax-ed at inference time.

Only the components needed for MiniFASNetV2 inference are included.
"""

import torch
from torch.nn import (
    BatchNorm1d, BatchNorm2d, Conv2d,
    Linear, Module, PReLU, Sequential,
)
...
class Depth_Wise(Module):
    def __init__(self, c1, c2, c3, residual=False,
        ...
        self.conv_dw = Conv_block(c2_in, c2_out, groups=c2_in,
...
class MiniFASNet(Module):
    def __init__(self, keep, embedding_size, conv6_kernel=(7, 7),
        ...
        c1 = [(keep[19], keep[20]), (keep[22], keep[23]),
              (keep[25], keep[26]), (keep[28], keep[29]),
              (keep[31], keep[32]), (keep[34], keep[35])]
        c2 = [(keep[20], keep[21]), (keep[23], keep[24]),
              (keep[26], keep[27]), (keep[29], keep[30]),
              (keep[32], keep[33]), (keep[35], keep[36])]
        c3 = [(keep[21], keep[22]), (keep[24], keep[25]),
              (keep[27], keep[28]), (keep[30], keep[31]),
...
_KEEP_V2 = [32, 32, 103, 103, 64, 13, 13, 64, 13, 13,
            64, 13, 13, 64, 13, 13, 64, 231, 231, 128,
            231, 231, 128, 52, 52, 128, 26, 26, 128, 77,
            77, 128, 26, 26, 128, 26, 26, 128, 308, 308,
...
def MiniFASNetV2(embedding_size=128, conv6_kernel=(7, 7),
    ...
    """MiniFASNetV2 — 80x80 input, ~0.044 GFLOPs, ~0.43 M params."""
    return MiniFASNet(_KEEP_V2, embedding_size,
                      conv6_kernel, drop_p, num_classes, img_channel)
