...
namespace ServiceLayer.Services
{
		...
		IBackgroundJobClient backgroundJobClient) : IAuthunticationService
	{
		...
		public async Task RegisterAsync(RegisterRequest request, IFormFile? file, CancellationToken cancellationToken = default)
		{
			...
			if (await DepartmentRepository.GetByIdAsync(request.DepartmentId, cancellationToken) is not { })
			...
			var user = new ApplicationUser
			{
				...
				AcademicYearId = (int)academicYear,
				IsEnrolled = true,
		...
		public async Task RevokeRefreshTokenAsync(string Token, string RefreshToken, CancellationToken cancellationToken = default)
		{
			var userId = _jwtProvider.ValidateToken(Token);
			if (userId is null)
		...
		public async Task SendResetPasswordCodeAsync(string email, CancellationToken cancellationToken = default)
		{
